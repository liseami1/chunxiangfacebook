...
//        changeNavigationBarApp(UINavigationController: navigationController!, changeTo: MakeNaviBarAppearance(color: .b1))
    ...
    /// Present
    func present<Content: View>(_ content: Content, animated: Bool = true, style: UIModalPresentationStyle = .formSheet, clearBack: Bool = false, detents: [UISheetPresentationController.Detent] = [.large()], needCoreDataEnvironment: Bool = false) {
        let HostController = clearBack ? FantasySwiftUIViewBox(content: content.background(ClearFullScreenBackView()), needCoreDataEnvironment: needCoreDataEnvironment) : FantasySwiftUIViewBox(content: content, needCoreDataEnvironment: needCoreDataEnvironment)
        HostController.modalPresentationStyle = style
        HostController.sheetPresentationController?.detents = detents
        present(HostController, animated: animated, style: style, detents: detents)
    }

    func present(_ vc: UIViewController, animated: Bool = false, style: UIModalPresentationStyle = .formSheet, detents: [UISheetPresentationController.Detent] = [.large()]) {
...
public func Present<Content: View>(_ content: Content, animated: Bool = true, style: UIModalPresentationStyle = .formSheet, clearBack: Bool = false, needCoreDataEnvironment: Bool = false, detents: [UISheetPresentationController.Detent] = [.large()]) {
    DispatchQueue.main.async {
        RouteStore.shared.present(content, animated: animated, style: style, clearBack: clearBack, detents: detents, needCoreDataEnvironment: needCoreDataEnvironment)
    }
}
